...
def build_adjacency_dict_and_leaf_counts(tree):
	adj = defaultdict(dict)
	leaf_count_dict = dict()
	
	for e in tree.edges.values():
		a, b, c, d = e.source_node_index, e.dest_node_index, e.first_char_index, e.last_char_index
		adj[a][b] = tree.string[c: d + 1]
	
	def leaf_count(node):
		if node in leaf_count_dict:
			return leaf_count_dict[node]
		if len(adj[node]) == 0:
			leaf_count_dict[node] = 1
			return 1
		res = sum(leaf_count(it) for it in adj[node])
		leaf_count_dict[node] = res
		return res
	leaf_count(0)
	
	return adj, leaf_count_dict
