...
fun <T> astSetEqual(a: Set<T>, b: Set<T>, message: String = "assertion failed: $DEFAULT_ERROR_MESSAGE") {
	if (a != b) {
		var diff = "elements in A\\B:\n" + a.minus(b).map { "\t${it.toString()}" }.joinToString(separator = "\n")
		diff += "\nelements in B\\A:\n" + b.minus(a).map { "\t${it.toString()}" }.joinToString(separator = "\n")
		throw MyAssertion("$message: set A (size ${a.size}) and set B (size ${b.size}) are not equal\n$diff")
	}
}
